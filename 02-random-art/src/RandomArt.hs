...
exprToString (Cube e)             = printf "(%s)^3" (exprToString (e))
exprToString (SorC e1 e2 e3) = printf "(%s<0?%s:%s)" (exprToString(e1)) (exprToString(e2)) (exprToString(e3))
...
eval x y e = case e of
             ...
             Cube e1 -> (eval x y e1) *  (eval x y e1) * (eval x y e1)
             Average e1 e2 -> ((eval x y e1) + (eval x y e2)) / 2 
             Times   e1 e2 ->  (eval x y e1) * (eval x y e2)
             SorC e1 e2 e3 -> if ((eval x y e1) < 0) == True then (eval x y e2) else (eval x y e3) 
             Thresh  e1 e2 e3 e4 -> if ((eval x y e1) < (eval x y e2)) == True then (eval x y e3) else (eval x y e4)
...
build 0
  ...
  where
    r         = rand 10
build d
        | 0 <= v && v < 10 = Sine(build (d-1))
        | 10 <= v && v < 20 = Cosine(build (d-1))
        | 20 <= v && v < 30 = Average (build (d-1)) (build (d-1))
        | 30 <= v && v < 40 = Times (build (d-1)) (build (d-1))
        | 50 <= v && v < 50 = Cube (build(d-1))
        | 60 <= v && v < 60 = SorC (build(d-1)) (build(d-1)) (build(d-1))
        | 40 <= v && v < 70 = Thresh (build (d-1)) (build (d-1)) (build (d-1)) (build (d-1))
