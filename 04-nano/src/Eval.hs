...
eval env (EIf e1 e2 e3) = if y == True then eval env e2 else eval env e3
	where 
	(VBool y) = eval env e1
...
eval env (EApp e1 argExpr) =
			   case eval env e1 of
                                VClos frozenEnv x body -> eval env' body
                                     where
                                     v = eval env argExpr
...
prelude =
  [ 
    ("head", VPrim(\(VPair x _) -> x)), ("tail", VPrim(\(VPair _ y) -> y))
